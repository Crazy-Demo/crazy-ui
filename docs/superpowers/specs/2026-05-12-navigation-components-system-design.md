...
export function useRouterLink(options: UseRouterLinkOptions) {
  ...
  const isActive = computed(() => {
    ...
    // 对象路由：优先比较 name，fallback 比较 path
    if (to.name) return currentRoute.name === to.name
    if (to.path) return currentRoute.path === to.path
    return false
    // 已知局限：不比较 query/params 的精确匹配
    // 企业后台场景中 name 匹配已足够，精确 query 匹配需业务自行处理
  })
...
### 2.3 受控/非受控值管理（useControllable）

Menu 和 Tabs 都需要"受控模式（外部传 active）+ 非受控模式（内部状态）"的统一管理。抽取为公共 hook：

```ts
// @company/ui-hooks/src/state/use-controllable/use-controllable.ts
...
export interface UseControllableOptions<T> {
  ...
  /** 值变更回调（emit update:xxx） */
  onChange: (value: T) => void
  /** 当 value 和 defaultValue 都未提供时的回退值 */
  fallback: T
}
...
| 决策点 | 选择 | 理由 |
|--------|------|------|
| 返回 WritableComputedRef | 读写统一 | 调用方可以 `active.value = index` 赋值，也可以在模板中读取，与普通 ref 使用方式一致 |
| 受控模式不修改内部状态 | 单向数据流 | 值由外部 prop 控制，内部修改无意义；只 emit 通知外部 |
| 非受控模式也调用 onChange | 双向通知 | 即使内部管理状态，也需要 emit 让外部知道值变了（用于 change 事件） |
| fallback 参数 | 确保类型安全 | 返回类型是 `T` 而非 `T | undefined`，调用方不需要判空 |
| `isControlled()` 是函数而非变量 | 支持动态切换 | prop 可能在运行时从 undefined 变为有值（如 v-model 绑定异步数据），函数调用保证实时判断 |

使用示例（与 useMenu/useTabs 中的调用方式对应）：

```ts
const active = useControllable({
  value: toRef(props, 'active'),
  defaultValue: toRef(props, 'defaultActive'),
  onChange: value => emit('update:active', value),
  fallback: '',
})

// 读取
console.log(active.value) // string | number

// 写入（受控模式会 emit，非受控模式会更新 internalValue + emit）
active.value = 'home'
```

### 2.4 键盘导航模式（Roving Focus）
...
/**
 * 在 items 原始数组中找到下一个可用项的索引
 * @param currentOriginalIndex 当前项在 items 中的索引
 * @param items 全部项目列表
 * @param loop 是否循环
 * @returns items 中的索引
 */
function findNextEnabled<T extends { disabled?: boolean }>(
  ...
  // 从当前项的下一个开始搜索
  for (let i = 1; i <= len; i++) {
    const idx = (currentOriginalIndex + i) % len
    // 非循环模式下，到达末尾就停止
    if (!loop && currentOriginalIndex + i >= len) return undefined
...
function findPrevEnabled<T extends { disabled?: boolean }>(
  ...
  // 从当前项的上一个开始搜索
  for (let i = 1; i <= len; i++) {
    const idx = ((currentOriginalIndex - i) % len + len) % len
    // 非循环模式下，到达开头就停止
    if (!loop && currentOriginalIndex - i < 0) return undefined
    if (!items[idx].disabled) return idx
  }
  return undefined
}
...
import type { ComponentSize, RouteLocation } from '@company/ui-core'
...
export const menuItemProps = {
  ...
  /** 路由跳转目标（框架无关路由描述） */
  to: { type: [String, Object] as PropType<string | RouteLocation> },
  /** 路由跳转是否替换 */
...
export const menuEmits = {
  ...
  /** 菜单项被点击时触发（无论是否改变 active） */
  'select': (index: string | number, indexPath: string[], item: MenuItemInstance) => true,
} as const
```

### 3.3.2 MenuItem Events
...
| 组件 | 插槽名 | 作用域 | 说明 |
|------|--------|--------|------|
| Menu | `default` | — | MenuItem / SubMenu / MenuItemGroup |
| MenuItem | `default` | — | 菜单项内容 |
| MenuItem | `icon` | — | 左侧图标 |
| SubMenu | `default` | — | 子菜单内容 |
| SubMenu | `title` | — | 子菜单标题 |
| MenuItemGroup | `default` | — | 分组内菜单项 |
| MenuItemGroup | `title` | — | 分组标题（不传 title prop 时使用） |
...
export function useMenu(props: MenuProps, emit: any) {
  ...
  function openSubMenu(index: string | number) {
    if (props.uniqueOpened) {
      // 一次性替换整个 Set，避免 clear + add 触发两次 watcher 导致闪烁
      openedSubMenus.value = new Set([index])
    } else {
      // 非 uniqueOpened 模式：在现有 Set 上添加
      const newSet = new Set(openedSubMenus.value)
      newSet.add(index)
      openedSubMenus.value = newSet
    }
  }
...
```ts
// SubMenu 弹出模式的核心逻辑
import { usePosition, useClickOutside, useTeleport, useEscapeKey, useOverlayManager } from '@company/ui-hooks'

const { container: teleportContainer, disabled: teleportDisabled } = useTeleport()
...
useEscapeKey({
  enabled: computed(() => isOpened.value),
  onEscape: () => closeSubMenu(props.index),
})
```
...
### 4.3.2 TabPane Events

TabPane 无自定义事件，其生命周期由 Tabs 控制。
...
| TabPane | `default` | — | 面板内容 |
| TabPane | `title` | — | 自定义标签标题（覆盖 title prop） |

### 4.3.4 editable 与 closable/addable 优先级

当 `editable=true` 时，`closable` 和 `addable` 的优先级规则：

```
closable 最终值 = props.closable ?? props.editable
addable 最终值 = props.addable ?? props.editable
```

即：`editable` 是快捷设置，显式传入的 `closable`/`addable` 优先。例如 `editable + closable=false` 表示可新增但不可关闭。
...
export function useTabBar(
  ...
  function update() {
    ...
    // 滚动偏移量：TabBar 需要减去当前滚动偏移，否则滚动后指示器位置错位
    const currentScrollOffset = scrollOffset?.value ?? 0

    if (isHorizontal) {
      barStyle.value = {
        width: `${tabEl.offsetWidth}px`,
        transform: `translateX(${tabEl.offsetLeft - currentScrollOffset}px)`,
      }
    } else {
      barStyle.value = {
        height: `${tabEl.offsetHeight}px`,
        transform: `translateY(${tabEl.offsetTop - currentScrollOffset}px)`,
      }
    }
  }
...
深色侧边栏场景——通过颜色 props 注入 CSS 变量，同时补充 hover/disabled 态的联动：
...
const style = computed(() => {
  ...
  if (props.backgroundColor) {
    styles['--ui-menu-bg-color'] = props.backgroundColor
    // 深色背景时自动调整 hover 态——基于背景色计算半透明叠加
    styles['--ui-menu-hover-bg-color'] = adjustAlpha(props.backgroundColor, 0.12)
  }
  if (props.textColor) {
    styles['--ui-menu-text-color'] = props.textColor
    // 深色文字时禁用态降低透明度
    styles['--ui-menu-disabled-color'] = adjustAlpha(props.textColor, 0.35)
  }
...
> 颜色 props 是 Component Token 的**运行时快捷覆盖**，不与 Token 管线冲突——它们在 DOM 上设置 CSS 变量，优先级等于 Token 生成的 CSS 变量（后设置的覆盖先设置的）。与 Token 的关系是：
>
> 1. Token 管线定义全局主题（`--ui-menu-bg-color: var(--ui-color-white)`）
> 2. 颜色 props 提供实例级覆盖（`<UiMenu background-color="#1d1e1f">`）
> 3. 优先级：props 覆盖 > Token 全局主题 > Token 默认值
>
> 为什么不提供 `theme` prop 接受 partial MenuToken：因为 Menu 的深色侧边栏场景只需要 3 个颜色变量（背景、文字、激活色），传一个 theme 对象反而增加了认知负担。但需要补全 hover/disabled 态的联动——设置深色背景后，默认的浅灰色 hover 会不可见，所以通过 `adjustAlpha` 自动计算合适的 hover 色。
>
> 其他组件（Input/Select/Checkbox）不需要这种快捷覆盖，因为它们在表单行内，颜色跟随全局主题；Menu 是唯一需要"同一个页面不同区域不同颜色方案"的组件。
...
```ts
{
  ...
  'menu-disabled-color': { value: '{color.text.disabled}' },
...
| 组件 | 角色 | aria-* 属性 | 说明 |
|------|------|-------------|------|
| Menu（horizontal） | `menubar` | `aria-orientation="horizontal"` | 顶栏菜单是菜单栏 |
| Menu（vertical） | `menu` | `aria-orientation="vertical"` | 侧边栏菜单是独立菜单 |
...
<ul role="menubar">
  <li role="none">
    <div role="menuitem" aria-expanded="false" aria-haspopup="menu">文件</div>
    <ul role="menu">
...
```
Menu / Tabs
  ├── @company/ui-core          (NavigationActiveProps、RouteLocation、Placement、ComponentSize)
  ├── @company/ui-hooks         (useNamespace、useRovingFocus、useRouterLink、useControllable、useResizeObserver)
  ├── @company/ui-icons         (图标：箭头、关闭）
  ├── @company/ui-theme         (CSS Variables、Component Token)
  └── @company/ui-utils         (DOM 工具、adjustAlpha)

Menu 额外依赖:
  └── overlay hooks             (usePosition、useClickOutside、useTeleport、useEscapeKey、useZIndex——SubMenu 弹出模式)
```
...
>
> **与 hooks 规范（Section 10.3）的关系**：hooks 规范要求"所有 injectionKey 定义在 @company/ui-core"。此规则的意图是让 hooks 和 vue-adapter 能共享同一个 key。但对于仅在组件树内部使用的 key（Menu→MenuItem、Tabs→TabPane），hooks 层不需要访问，vue-adapter 也不需要 provide，因此放在 core 是不必要的耦合。输入组件规范中 `checkboxGroupInjectionKey` / `radioGroupInjectionKey` 也采用了同样的例外处理（见附录 M19）。
>
> 判断标准：**如果 hooks 包或 vue-adapter 包需要 import 这个 key，它必须在 core；如果只有组件内部使用，可以留在组件内。**
...
> `RouteLocation` 定义在 core 层而不是直接 import vue-router 的类型，确保不安装 vue-router 的项目也能编译通过。`useRouterLink` 在 hooks 层负责将 `RouteLocation` 转换为 vue-router 的 `router.push()` 调用——组件和 core 层完全不知道 vue-router 的存在。
